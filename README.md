...
| CVE 조회 | NVD API | 1-day 취약점 자동 검색 |
...
### 대안: Non-root 실행 (Linux만)

Root로 실행할 수 없는 경우, setcap을 설정하세요:

```bash
# 1. setcap 설정 (한 번만)
sudo ./setup.sh  # 옵션 2 선택

# 2. 일반 사용자로 실행
python main.py --input targets.txt --verbose
```

> ⚠️ macOS는 setcap을 지원하지 않습니다. sudo로 실행하세요.
...
Input Parser → Subdomain Scanner → Host Discovery → Port Scanner
                                                           ↓
                      Report ← Screenshot ← Nuclei ← Dir Scanner ← CVE Lookup ← Tech Detector
```
